...
public class prQuadtree<T extends Compare2D<? super T>> {
	...
	private String print(String descriptor, prQuadNode node, String prefix,
		...
		if (this.isLeaf(node)) {
			...
			prQuadInternal internal = (prQuadInternal) node;
			output += this.print("NE", internal.NE, prefix, ((internal.NW == null)
			    && (internal.SW == null) && (internal.SE == null)), expanded);
			output += this.print("NW", internal.NW, prefix,
			    ((internal.SW == null) && (internal.SE == null)), expanded);
			output += this.print("SW", internal.SW, prefix, ((internal.SE == null)),
			    expanded);
